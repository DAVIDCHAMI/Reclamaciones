...
Característica: Realizar pago de un siniestro
  ...
  Esquema del escenario: Crear cheque con múltiples pagos a diferentes líneas de reserva de un siniestro autos
    ...
    Cuando se genere un pago por siniestro de auto <Tipo de pago> al beneficiario <Beneficiario del pago> por el medio de pago de <Método de Pago> sobre las líneas de reserva <Línea de reserva 1> y <Línea de Reserva 2> afectando la cobertura de <Pago Solo Sura> es Sura con una retención de <Código de retención pago> <Número de vehículos involucrados del tercero en el siniestro>
    Entonces se genera una orden de pago para que le sea entregado al usuario
