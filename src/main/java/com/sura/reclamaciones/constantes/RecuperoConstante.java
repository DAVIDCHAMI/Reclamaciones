package com.sura.reclamaciones.constantes;

public class RecuperoConstante {

  public static final String OTROS = "Otros";
  public static final String RECUPERO = "Recuperación";
  public static final String CANTIDAD = "Cantidad";
  public static final String CODIGO_RETENCION = "Código de retención";
  public static final int TIEMPO_COLA= 120;
  public static final String PAIS= "Country-inputEl";
  public static final String DEPARTAMENTO = "State-inputEl";
  public static final String CIUDAD = "City-inputEl";
  public static final String LINEA_RESERVA = "ReserveLine-inputEl";
}
